...
import { CalentarItemProps } from './CalendarItem.interfaces';
import { TaskType } from 'redux/MainReducer/mainReducer.interfaces';

export const CalendarItem: React.FC<CalentarItemProps> = ({
  tasks,
  ...
  const [completedTasksExist, setCompletedTasksExists] =
    useState<boolean>(false);
  const [incompletedTasksExist, setIncomletedTasksExists] =
    useState<boolean>(false);

  const hasCompletedTask = (tasks: { [key: string]: TaskType }) => {
    return Object.values(tasks).filter((e) => e.isDone).length > 0
      ? true
      : false;
  };
  ...
  return (
    <div className={s.container}>
      <div
        onClick={() => onCalendarItemChanged(date)}
        className={cn({ [s.today]: today }, s.calendarItem, {
          [s.active]: active,
        })}
      >
